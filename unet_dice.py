...
class myUnet(object):
    ...
    def train(self):
        ...
        model = self.get_unet()
        print("got unet")
        model.load_weights('/home/ubuntu/unet-master-DWI_latest/weights.h5')
        model_checkpoint = ModelCheckpoint('/home/ubuntu/unet-master-DWI_latest/weights.h5', monitor='loss',verbose=1, save_best_only=True)
        print('Fitting model...')
        model.fit(imgs_train, imgs_mask_train, batch_size=10, epochs=5, verbose=1,validation_split=0.3, shuffle=True, callbacks=[model_checkpoint])

        print('predict test data')
        model.load_weights('/home/ubuntu/unet-master-DWI_latest/weights.h5')
        imgs_mask_test = model.predict(imgs_test, batch_size=1, verbose=1)
        np.save('/home/ubuntu/unet-master-DWI_latest/results/imgs_mask_test.npy', imgs_mask_test)
    ...
    def save_img(self):
        ...
        imgs = np.load('/home/ubuntu/unet-master-DWI_latest/results/imgs_mask_test.npy')
        for i in range(imgs.shape[0]):
            img = imgs[i]
            img = array_to_img(img)
            img.save("/home/ubuntu/unet-master-DWI_latest/results/"+str(i+137)+".jpg")
